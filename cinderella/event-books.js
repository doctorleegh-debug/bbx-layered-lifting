...
document.querySelectorAll('[data-event-books]').forEach(root=>{
 ...
 async function turn(){timer=null;if(!canRun()||pending||transition){schedule();return}const bookSlot=mobile.matches?0:slot,book=books[bookSlot];slot=(slot+1)%books.length;const next=(indices[bookSlot]+(mobile.matches?1:books.length))%items.length;if(next===indices[bookSlot]){schedule();return}const item=items[next],token=serial,img=warmed?.url===item.image?warmed.image:new Image();pending=img;if(!img.src)img.src=item.image;
  ...
  const anim=leaf.animate([{transform:'rotateY(0deg)',opacity:1,offset:0},{transform:'rotateY(-105deg)',opacity:1,offset:.66},{transform:'rotateY(-165deg)',opacity:0,offset:1}],{duration:turnDuration,easing:'cubic-bezier(.35,0,.25,1)',fill:'forwards'});
 ...
 function applyLive(){if(!liveItems||root.contains(document.activeElement))return;stop();items=liveItems;liveItems=null;slot=0;books.forEach((b,i)=>{b.hidden=i>=items.length;if(!b.hidden){indices[i]=i;update(b,items[i],i)}});root.dataset.ebLive='connected';root.dataset.ebUpdatedAt=new Date().toISOString();root.dispatchEvent(new CustomEvent('bb:eventsupdated',{detail:{count:items.length}}));schedule()}
